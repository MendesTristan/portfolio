    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-background/80 backdrop-blur-xl border-b border-card-border shadow-lg shadow-black/10" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3.5 flex items-center justify-between">
          ...
          <ul className="flex items-center gap-1">
            {navLinks.map(({ href, label }) => (
              <li key={href}>
                <a
                  href={href}
                  className={`relative text-sm font-medium px-4 py-2 rounded-lg transition-all duration-200 ${active === href ? "text-primary-light bg-primary/10" : "text-text-muted hover:text-text-primary hover:bg-white/[0.03]"}`}
                >
                  {label}
                </a>
              </li>
        ...
        <div className="flex items-center gap-2 md:hidden">
          <LanguageSwitcher />
          <button onClick={() => setOpen(!open)} className="w-8 h-8 flex flex-col items-center justify-center gap-1.5" aria-label="Menu">
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ${open ? "max-h-96" : "max-h-0"}`}>
        <ul className="px-4 sm:px-6 pb-5 space-y-1 bg-background/95 backdrop-blur-xl">
          {navLinks.map(({ href, label }) => (
            <li key={href}>
              <a
                href={href}
                onClick={() => setOpen(false)}
                className={`block text-base font-medium px-3 py-2.5 rounded-lg transition-all ${active === href ? "text-primary-light bg-primary/10" : "text-text-secondary"}`}
              >
                {label}
              </a>
            </li>

...
const MissionList = ({ missions }) => (
  <div className="mb-4">
    <h4 className="text-xs font-mono uppercase tracking-[0.15em] text-text-muted mb-2">Missions</h4>
    <ul className="space-y-3">
      {missions.map((m, i) => (
        <li key={i} className="flex gap-2 text-sm text-text-secondary leading-relaxed"><CheckIcon /><span>{m}</span></li>
      ))}
    </ul>
  </div>
);
...
const HighlightBar = ({ highlights }) => (
  <div className="mb-4">
    <h4 className="text-xs font-mono uppercase tracking-[0.15em] text-text-muted mb-2">Impact</h4>
    <div className={`grid grid-cols-1 gap-2 ${highlights.length <= 3 ? "sm:grid-cols-3" : "sm:grid-cols-2 lg:grid-cols-4"}`}>
      {highlights.map((h, i) => (
        <div key={i} className="rounded-xl px-3 py-2.5 bg-primary/[0.05] border border-primary/10 transition-all duration-300 hover:border-primary/20">
          <p className="text-xl font-bold gradient-text leading-none mb-1">{h.metric}</p>
          <p className="text-xs font-semibold text-text-primary">{h.label}</p>
          <p className="text-xs text-text-muted leading-snug">{h.detail}</p>
        </div>
      ))}
    </div>
  </div>
...
const EnrichedCard = ({ exp }) => (
  <>
    <p className="text-text-secondary text-sm leading-relaxed mb-4 border-l-2 border-primary/20 pl-3 italic">{exp.context}</p>
    {exp.missions && <MissionList missions={exp.missions} />}
    {exp.highlights && <HighlightBar highlights={exp.highlights} />}
    <div className="flex flex-wrap gap-2 mt-1">{exp.stack.map((t, j) => <span key={j} className={TAG_CLS}>{t}</span>)}</div>
  </>
);
...
const SimpleCard = ({ exp }) => (
  <>
    ...
    <div className="flex flex-wrap gap-2">{exp.stack.map((t, j) => <span key={j} className={TAG_CLS}>{t}</span>)}</div>
  </>
);
    ...
    <SectionWrapper id="experience" title="Expérience professionnelle" subtitle="Parcours en finance de marché, data et développement.">
      ...
      <div ref={inner} className="max-w-5xl mx-auto relative">
        ...
        {experiences.map((exp, i) => {
          ...
          return (
            <div key={i} data-gsap className="exp-card relative flex gap-5 md:gap-7 mb-8 last:mb-0">
              ...
              <div className="flex-1 rounded-xl p-6 bg-white/[0.02] border border-white/[0.05] transition-all duration-300 hover:border-primary/20 group">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                  ...
                  <span className="text-text-muted text-xs font-mono bg-white/[0.04] px-3 py-1 rounded-full whitespace-nowrap shrink-0">{exp.period}</span>
                </div>
